...
#160 Alegria = 70% Total de 112 frases para treinamento | 30%  Total de 48 frases para teste
#120 Desgosto = 70% Total de 84 frases para treinamento | 30%  Total de 36 frases para teste
#120 Medo = 70% Total de 84 frases para treinamento | 30%  Total de 36 frases para teste
#120 Raiva = 70% Total de 84 frases para treinamento | 30%  Total de 36 frases para teste
#120 Supresa = 70% Total de 84 frases para treinamento | 30%  Total de 36 frases para teste
#120 Tristeza = 70% Total de 84 frases para treinamento | 30%  Total de 36 frases para teste

#Base de Teste para testar o algoritmo
...
# Retirar as stoprwords do próprio NLTK
...
# Capturar as Stemmings
def aplicastmmer(texto):
...
frasescomstemmingtreinamento = aplicastmmer(basetreinamento)
frasescomstemmingteste = aplicastmmer(baseteste)
#print(frasescomstemming)

#Listagem de todas as palavras que existe no banco de dados
...
palavrasTreinamento = buscarPalavras(frasescomstemmingtreinamento)
palavrasTeste = buscarPalavras(frasescomstemmingteste)


#Eliminar as repetições dos radicais

def buscarFrequencia(palavras):
    palavras = nltk.FreqDist(palavras)
    return palavras

frequenciaTreinamento = buscarFrequencia(palavrasTreinamento)
frequenciaTeste = buscarFrequencia(palavrasTeste)
#print(frequencia.most_common(50))

# Busca das palavras unicas

def buscaPalavrasUnicas(frenquencia):
    freq = frequenciaTreinamento.keys()
    return freq
...
basecompletaTreinamento = nltk.classify.apply_features(extratoPalavras, frasescomstemmingtreinamento)
basecompletaTeste = nltk.classify.apply_features(extratoPalavras,frasescomstemmingteste)
...
#Dectar o tipo de erro o qual foi classificado na base de teste
erros = []
...
teste = input('Digite uma frase para analisar sua polaridade:')
testestemming = []
stemmer = nltk.stem.RSLPStemmer()
...
novo = extratoPalavras(testestemming)


sentimento = classificador.classify(novo)

print('A emoção presente nesta frase segundo Ekman é: '+sentimento)

#Detectar a probalidade de positivo negativo ou positivo
